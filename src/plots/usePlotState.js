...
export function usePlotState(plotLabel, logAction) {
  ...
  const handlePlotTypeChange = (newPlotType) => {
    ...
    setYVars([]);
    if (logAction) {
      logAction('plot_interaction', 'type_changed', { plotLabel, newType: newPlotType });
    }
  };
  ...
  const handleXVariableToggle = (variable) => {
    ...
    setXVars(newXVars);
    if (logAction) {
      logAction('plot_interaction', 'x_variable_toggled', { plotLabel, variable, selected: !isSelected });
    }
  };
  ...
  const handleYVariableToggle = (variable) => {
    ...
    setYVars(newYVars);
    if (logAction) {
      logAction('plot_interaction', 'y_variable_toggled', { plotLabel, variable, selected: !isSelected });
    }
  };
  ...
  const handleHistogramXVariableToggle = (variable) => {
    ...
    setXVars(newXVars);
    if (logAction) {
      logAction('plot_interaction', 'histogram_x_variable_toggled', { plotLabel, variable, selected: !isSelected });
    }
  };
  ...
  const handlePieVariableSelect = (variable) => {
    ...
    if (logAction) {
      logAction('plot_interaction', 'pie_variable_selected', { plotLabel, variable });
    }
  };
  ...
  const handleCadetFilterToggle = (name) => {
    ...
    if (logAction) {
      logAction('plot_interaction', 'cadet_filter_toggled', { plotLabel, cadet: name, selected: newValue });
    }
  };
  ...
  const onSelectAllCadets = () => {
    ...
    if (logAction) {
      logAction('plot_interaction', 'cadet_filter_select_all', { plotLabel });
    }
  };
  const onDeselectAllCadets = () => {
    setCadetFilter(playerNames.reduce((acc, name) => ({ ...acc, [name]: false }), {}));
    if (logAction) {
      logAction('plot_interaction', 'cadet_filter_deselect_all', { plotLabel });
    }
  };
